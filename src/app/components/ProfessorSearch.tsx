...
import { Search, Clock, MapPin, Video, User, Calendar, AlertCircle } from 'lucide-react';
...
const ProfessorSearch: React.FC<ProfessorSearchProps> = ({ onBookSlot, currentUserId }) => {
  ...
  const days: (DayOfWeek | 'Todos')[] = ['Todos', 'Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes'];
  ...
  const loadData = async () => {
    try {
      ...
      const professorIds = [...new Set(slots?.map(slot => slot.professor_id) || [])];
      ...
      // Combinar datos
      const slotsWithProfessors = (slots || []).map(slot => ({
        ...slot,
        professor: professorMap.get(slot.professor_id)
      }));

      // Si hay un usuario actual, verificar qué slots ya están reservados
      ...
      if (currentUserId) {
        ...
        if (!appointmentsError && appointments) {
          bookedSlotIds = appointments.map(apt => apt.slot_id);
        }
      }

      // Marcar los slots que ya están reservados
      const slotsWithBookedStatus = (slots || []).map(slot => ({
        ...slot,
        isBooked: bookedSlotIds.includes(slot.id)
      }));

      setAvailableSlots(slotsWithBookedStatus);
      setFilteredSlots(slotsWithBookedStatus);
    } catch (error: any) {
  ...
  const filterSlots = () => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(slot => 
        slot.professor?.username.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }
    ...
    if (selectedModality !== 'Todos') {
      filtered = filtered.filter(slot => slot.modalities.includes(selectedModality));
    }
  ...
  const groupSlotsByProfessor = () => {
    const grouped: { [key: string]: { professor: Professor; slots: AppointmentSlot[] } } = {};
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-xl text-gray-600 dark:text-gray-300">Cargando profesores...</div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg">
        <h3 className="text-xl font-semibold mb-4 text-gray-800 dark:text-white">Buscar Horarios Disponibles</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <select
              value={selectedDay}
              onChange={(e) => setSelectedDay(e.target.value as DayOfWeek | 'Todos')}
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setSelectedModality(e.target.value as Modality | 'Todos')}
              className="w-full px-4 py-2 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition"
            >
      ...
      {groupedData.length === 0 ? (
        ...
        <div className="space-y-6">
          {groupedData.map(({ professor, slots }) => (
            <div key={professor.id} className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg">
...
const SlotCard: React.FC<SlotCardProps> = ({ slot, onBookSlot }) => {
  const [selectedModality, setSelectedModality] = useState<Modality>(slot.modalities[0]);
  ...
  return (
    <>
      <div className={`bg-gray-50 dark:bg-gray-700 p-4 rounded-xl border-2 transition-all ${
        slot.isBooked 
        ...
        <div className="space-y-2 text-sm text-gray-600 dark:text-gray-300 mb-4">
          <div className="flex items-center gap-2">
            ...
            <span>{slot.start_time} ({slot.duration} min)</span>
          </div>
          <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            {slot.modalities.includes('Presencial') && <User className="w-4 h-4 text-green-600" />}
            {slot.modalities.includes('Online') && <Video className="w-4 h-4 text-purple-600" />}
        ...
        >
          ...
          >
            ...
            <div className="space-y-3 mb-6 text-gray-700 dark:text-gray-300">
              <p><strong>Profesor:</strong> {slot.professor?.username}</p>
              <p><strong>Día:</strong> {slot.day}</p>
              ...
              <p><strong>Ubicación:</strong> {slot.location}</p>
            </div>
